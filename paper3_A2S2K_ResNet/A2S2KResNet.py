...
model = S3KAIResNet(BAND, CLASSES_NUM, 2).to(device)

summary(
    model,
    input_size=(1, img_rows, img_cols, BAND),
...
manual_report_path = (
    "./report/manual_result_"
    + "S3KAIResNetpatch_" + str(img_rows)
    + "_" + Dataset
    + "_split_" + str(VALIDATION_SPLIT)
    + "_lr_" + str(lr)
    + "_" + PARAM_OPTIM
...
with open(manual_report_path, "w", encoding="utf-8") as f:
    f.write("A2S2K-ResNet Reproduction Result\n")
    f.write("=" * 60 + "\n")
    f.write(f"Dataset: {Dataset}\n")
    f.write(f"Patch size: {img_rows} x {img_cols}\n")
    f.write(f"Kernel number: {PARAM_KERNEL_SIZE}\n")
    f.write(f"Optimizer: {PARAM_OPTIM}\n")
    f.write(f"Learning rate: {lr}\n")
    f.write(f"Split ratio: {VALIDATION_SPLIT}\n")
    ...
    if len(OA) > 0:
        f.write("Summary:\n")
        f.write(f"Mean OA: {np.mean(OA):.6f}\n")
        f.write(f"Std OA : {np.std(OA):.6f}\n")
        f.write(f"Mean AA: {np.mean(AA):.6f}\n")
        f.write(f"Std AA : {np.std(AA):.6f}\n")
        f.write(f"Mean Kappa: {np.mean(KAPPA):.6f}\n")
        f.write(f"Std Kappa : {np.std(KAPPA):.6f}\n\n")

    f.write("Per-class accuracy:\n")
    f.write(str(ELEMENT_ACC) + "\n\n")
...
print(f"[INFO] Manual result saved to: {manual_report_path}")
